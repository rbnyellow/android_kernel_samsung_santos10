...
{
	uint64_t *valuep = container_of(file->f_dentry->d_inode,
		struct proc_inode, vfs_inode)->pde->data;
...
static int read_proc_bool(struct file *file, char __user *buf,
			  size_t size, loff_t *ppos)
{
	bool *valuep = container_of(file->f_dentry->d_inode,
		struct proc_inode, vfs_inode)->pde->data;
	char tmp[24];
	size_t tmp_size;
...
/* 
 * SRI-N: b.unnithan: Fix CTS TC Failure android.net.cts.TrafficStatsTest --testTrafficStatsForLocalhost
 */
	if (entry != NULL) {
