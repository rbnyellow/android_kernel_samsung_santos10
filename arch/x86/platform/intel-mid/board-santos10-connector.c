...
struct intel_host_noti_init {
	struct delayed_work noti_init_work;
...
#endif
	struct intel_host_noti_init h_init;
};
...
static void sec_santos10_cable_event_worker(struct work_struct *work)
{
	...
	case EXTCON_USB_HOST:
		mutex_lock(&otg->h_init.noti_lock);
		if (otg->h_init.host_noti_init)
			santos10_detected_otg(otg, cable->cable_state);
		otg->h_init.host_start_call = cable->cable_state;
		mutex_unlock(&otg->h_init.noti_lock);
		break;
...
static void host_check_init_work(struct work_struct *work)
{
	struct intel_host_noti_init *n_init = container_of
		(work, struct intel_host_noti_init, noti_init_work.work);
	struct santos10_otg *santos_otg =
			    container_of(n_init, struct santos10_otg, h_init);

	mutex_lock(&n_init->noti_lock);
	if (n_init->host_start_call)
		santos10_detected_otg(santos_otg, 1);
	n_init->host_noti_init = 1;
	mutex_unlock(&n_init->noti_lock);
}

static void santos10_host_noti_init(struct santos10_otg *otg)
{
	INIT_DELAYED_WORK(&otg->h_init.noti_init_work,
					host_check_init_work);
	mutex_init(&otg->h_init.noti_lock);
	schedule_delayed_work(&otg->h_init.noti_init_work,
...
int __init sec_santos10_usb_init(void)
{
	...
	santos10_host_noti_init(santos_otg);
...
int __init sec_santos10_max77693_gpio_init(void)
{
	...
	ret = sysfs_create_link(&santos_otg->switch_dev->kobj, &edev->dev->kobj, "muic");
	if (ret) {
		pr_err("%s: fail(%d) to create muic link\n", __func__, ret);
		return ret;
	}
