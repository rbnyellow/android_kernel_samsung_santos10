...
static atomic_t cpufreq_log_idx[NR_CPUS] = {
	ATOMIC_INIT(-1), ATOMIC_INIT(-1),
	ATOMIC_INIT(-1), ATOMIC_INIT(-1)
};
...
void __sec_debug_cpufreq_log(int cpu, int freq)
{
	unsigned int i;
	...
	i = atomic_inc_return(&cpufreq_log_idx[cpu]) &
	    (ARRAY_SIZE(psec_debug_log->cpuidles[0]) - 1);
	psec_debug_log->cpufreqes[cpu][i].time = cpu_clock(cpu);
	psec_debug_log->cpufreqes[cpu][i].freq = freq;
}
