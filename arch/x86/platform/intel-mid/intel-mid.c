...
void intel_scu_devices_create(void)
{
	...
	for (i = 0; i < ipc_next_dev; i++)
		if (unlikely(platform_device_add(ipc_devs[i]))) {
			pr_err("failed to add ipc_devs(%d) platform data\n", i);
			return ;
		}
...
void intel_psh_devices_create(void)
{
	...
	if (unlikely(platform_device_add(psh_ipc))) {
		pr_err("failed to add psh_ipc platform data\n");
		platform_device_put(psh_ipc);
		return;
	}
}
...
void __init install_irq_resource(struct platform_device *pdev, int irq)
{
	...
	res.start = irq;
	if (unlikely(platform_device_add_resources(pdev, &res, 1)))
		pr_err("failed to add install_irq_resource platform data\n");
}
...
static int __init intel_psh_ipc_subdev_init(void)
{
	...
	if (unlikely(platform_device_add(psh))) {
		pr_err("failed to add psh platform data\n");
		platform_device_put(psh);
		return -1;
	}
