...
static void resume_hsu_port_enable(int port)
{
	struct mfld_hsu_info *info = platform_hsu_info + port;
...
int intel_mid_hsu_init(int port, struct device *dev, irq_handler_t wake_isr)
{
	...
	if (info->wake_gpio)
		if (unlikely(gpio_request(info->wake_gpio, "hsu")))
			pr_err("Request for hsu(%d) GPIO fails\n", info->wake_gpio);
	if (info->rx_gpio)
		if (unlikely(gpio_request(info->rx_gpio, "hsu")))
			pr_err("Request for hsu(%d) GPIO fails\n", info->rx_gpio);
	if (info->tx_gpio)
		if (unlikely(gpio_request(info->tx_gpio, "hsu")))
			pr_err("Request for hsu(%d) GPIO fails\n", info->tx_gpio);
	if (info->cts_gpio)
		if (unlikely(gpio_request(info->cts_gpio, "hsu")))
			pr_err("Request for hsu(%d) GPIO fails\n", info->cts_gpio);
	if (info->rts_gpio)
		if (unlikely(gpio_request(info->rts_gpio, "hsu")))
			pr_err("Request for hsu(%d) GPIO fails\n", info->rts_gpio);
