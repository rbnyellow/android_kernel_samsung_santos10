				...
				struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(muic_regs); i++) {
		ret = max77693_read_reg(gInfo->max77693->regmap_muic,
					muic_regs[i], &val);
		if (unlikely(ret)) {
			pr_err("error(%d): Fail to read max77693 register: %x",
					ret, muic_regs[i]);
			continue;
		}

		off += sprintf(buf + off, "REG=0x%x, VAL=0x%x\n",
				muic_regs[i], val);
	}
...
static void _max77693_muic_set_ctrl3(struct max77693_muic_info *info)
{
	u8 val;
	int ret;
...
void max77693_muic_set_ctrl3(void)
{
	_max77693_muic_set_ctrl3(gInfo);
}
EXPORT_SYMBOL(max77693_muic_set_ctrl3);
 ...
 */
bool max77693_check_muic_reset(void)
{
	struct max77693_muic_info *info = gInfo;
	u8 intmask1;
	u8 intmask2;
	...
	max77693_read_reg(info->max77693->regmap_muic,
			MAX77693_MUIC_REG_INTMASK1, &intmask1);
	max77693_read_reg(info->max77693->regmap_muic,
			MAX77693_MUIC_REG_INTMASK2, &intmask2);
	if (!intmask1 || !intmask2) {
		pr_info("MUIC block is reset (%x,%x)\n", intmask1, intmask2);
		return true;
	}

	return false;
}
EXPORT_SYMBOL(max77693_check_muic_reset);
...
static void _detected(struct max77693_muic_info *info, u32 new_state)
{
	...
	/* 1. report detachment event */
	for (index = 0; index < SUPPORTED_CABLE_MAX; index++) {
		if (!(changed_state & BIT(index)))
			continue;

		attach = (new_state & BIT(index)) ? true : false;

		/* skip attachment event.. */
		if (attach)
			continue;
		...
		if (unlikely(ret < 0))
			dev_err(info->dev,
				"fail to notify %s(%d) attachment(%d)\n",
				extcon_cable_name[index], index, ret);
	}

	/* 2. report attachment event */
	for (index = 0; index < SUPPORTED_CABLE_MAX; index++) {
		if (!(changed_state & BIT(index)))
			continue;
		...
		if (unlikely(ret < 0))
			dev_err(info->dev,
				"fail to notify %s(%d) detachment(%d)\n",
				extcon_cable_name[index], index, ret);
	}
				       ...
				       u8 status1, u8 status2)
{
	...
	/* Workaround: error to detect correct adc value causes
	 * faling to detect TA detachment event.
	 * This workaround helps to detect TA detachment using
	 * vbus and charger type.
	 */
	if ((chgtyp == CHGTYP_NO_VOLTAGE || !vbvolt) &&
			(pre_state & BIT(EXTCON_TA))) {
		dev_info(info->dev, "assume ta detachment\n");
		_detected(info, 0);
	}
	...
	case ADC_JIG_USB_OFF:
		switch (chgtyp) {
			/* workaround for non-samsung USB cable
			 * : to distinguish the non-samsung USB cable from
			 * JIG_USB_OFF by using CHGTYP value, we should
			 * temporarily disable accdet when the CHGTYP is zero.
			 * (accdet will be enabled when the cable is detached)
			 */
	...
	case ADC_POWER_SHARING:
			new_state = BIT(EXTCON_POWER_SHARING);
		break;
	case ADC_CEA936ATYPE1_CHG:
		/* workaround for Specific Non-samsung USB cable */
...
static int max77804_suspend(struct device *dev)
{
	struct max77693_muic_info *info = dev_get_drvdata(dev);
	if (info)
		max77693_muic_set_path(info, PATH_OPEN);
	else
		dev_err(dev, "%s: dev_get_drvdata fail\n", __func__);
...
static int max77804_resume(struct device *dev)
{
	struct max77693_muic_info *info = dev_get_drvdata(dev);
	if (info)
		max77693_muic_set_path(info, info->path);
	else
		dev_err(dev, "%s: dev_get_drvdata fail\n", __func__);
