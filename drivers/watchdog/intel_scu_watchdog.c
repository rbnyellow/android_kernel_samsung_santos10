...
static int shutdown_pre_timeout = 10;
static int shutdown_timeout = 160;

static struct rpmsg_instance *watchdog_instance;

#ifdef CONFIG_KERNEL_THREAD
			   ...
			   void *another_unused)
{
	...
	timer_timeout = DEFAULT_TIMER_DURATION;
...
int write_security(struct file *file, const char __user *buff,
...
static int create_debugfs_entries(void)
{
	...
	if (!dev->dfs_kwd_reset_type) {
		pr_err(PFX "%s: Error, cannot create kwd reset_type file\n",
			__func__);
		goto error;
	}
...
#if defined(CONFIG_DISABLE_SCU_WATCHDOG) && !defined(CONFIG_KERNEL_THREAD)
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = watchdog_config_and_start(shutdown_timeout, shutdown_pre_timeout);
	if (ret)
...
#ifdef CONFIG_KERNEL_THREAD
static int watchdog_keep_task(void *data)
{
	int ret;
	...
	sched_setscheduler_nocheck(watchdog_task, SCHED_FIFO, &param);

	get_task_struct(watchdog_task);
	set_freezable();

	while (true) {

		if (kthread_should_stop()) {
			pr_warn(PFX"Stop ia_watchdog thread!!!!\n");
			intel_scu_stop();
			break;
		}

		if (watchdog_device.shutdown_flag == true) {
			pr_warn(PFX"do nothing if we are shutting down\n");
			goto do_nothing;
		}
...
static int intel_scu_watchdog_init(void)
{
	...
	if (xen_start_info) {
		...
		ret = xen_register_gsi(watchdog_device.timer7_tbl_ptr->irq, -1, 0, 1);
		if (ret < 0) {
			pr_err(PFX "error binding irq %d\n",
					watchdog_device.timer7_tbl_ptr->irq);
			goto error_misc_register;
		}

		ret = xen_register_gsi(watchdog_device.timer6_tbl_ptr->irq, -1, 0, 1);
		if (ret < 0) {
			pr_err(PFX "error binding irq %d\n",
...
#ifdef CONFIG_KERNEL_THREAD
	if (!disable_kernel_watchdog) {
		watchdog_task = kthread_create(watchdog_keep_task, NULL,
				"ia_watchdog");
