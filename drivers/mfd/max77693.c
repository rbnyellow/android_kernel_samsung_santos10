...
#define PMIC_REV(_val)  ((_val) & 0x07)
#define PMIC_VER(_val)  (((_val) & 0xf8) >> 3)
...
int max77693_bulk_read(struct regmap *map, u8 reg, int count, u8 *buf)
{
	...
	if (unlikely(ret < 0)) {
		pr_err("%s: error: reg=0x%x, count=%d, ret=%d\n",
...
int max77693_write_reg(struct regmap *map, u8 reg, u8 value)
{
	...
	if (unlikely(ret < 0)) {
		pr_err("%s: error: reg=0x%x, value=0x%x, ret=%d\n",
...
int max77693_update_reg(struct regmap *map, u8 reg, u8 val, u8 mask)
{
	...
	if (unlikely(ret < 0)) {
		pr_err("%s: error: reg=0x%x, value=0x%x, mask=0x%x, ret=%d\n",
				__func__, reg, val, mask, ret);
		return ret;
	}
...
static const struct regmap_config max77693_haptic_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (pdata->get_irq_gpio) {
		max77693->irq_gpio = pdata->get_irq_gpio();
		if (unlikely(max77693->irq_gpio < 0))
			goto err_read;
	}
	dev_info(max77693->dev, "irq_gpio = %d, irq_base = %d\n",
				max77693->irq_gpio, max77693->irq_base);
	...
	dev_info(max77693->dev, "REVISION=%d, VERSION=%d\n",
				max77693->pmic_rev, max77693->pmic_ver);

	ret = max77693_read_reg(max77693->regmap, MAX77693_PMIC_REG_MAINCTRL1,
				&reg_data);
	if (ret < 0) {
		dev_err(max77693->dev, "failed to read MANCTRL1 register.\n");
		goto err_read;
	}
	dev_info(max77693->dev, "MANCTRL1 value = 0x%x\n", reg_data);

	max77693_update_reg(max77693->regmap, MAX77693_CHG_REG_SAFEOUT_CTRL,
			0x00, 0x30);
	...
	pdata->regmap_max77693 = max77693->regmap;
	pdata->regmap_max77693_haptic = max77693->regmap_haptic;
