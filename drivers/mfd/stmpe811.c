...
int stmpe811_adc_get_value(u8 channel)
{
	...
	if (ret < 0) {
		pr_err("Failed to get stmpe811 adc value(id=%d)", adc->id);
		goto adc_get_fail;
	}
	...
	while (1) {
		...
		count++;
		if (count > 10) {
			pr_err("timeout to get stmpe811 adc value(id=%d)",
					adc->id);
			goto adc_get_fail;
		}
	}
...
static ssize_t stmpe811_regs_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("STMPE811 registers\n");
	for (i = 0; i < ARRAY_SIZE(regs8); i++) {
		stmpe811_i2c_read(stmpe811_adc_i2c_client, regs8[i], data, 1);
		off += sprintf(buf + off, "REG: 0x%x, VAL: 0x%x\n",
				regs8[i], data[0]);
	}
	for (i = 0; i < ARRAY_SIZE(regs16); i++) {
		stmpe811_i2c_read(stmpe811_adc_i2c_client, regs16[i], data, 1);
		off += sprintf(buf + off, "REG: 0x%x, VAL: 0x%x,%x\n",
				regs16[i], data[0], data[1]);
	}

	return off;
}

static DEVICE_ATTR(regs, S_IRUGO, stmpe811_regs_show, NULL);
...
static int __init stmpe811_adc_init(void)
{
	...
	ret = device_create_file(sec_adc_dev, &dev_attr_regs);
	if (ret < 0) {
		pr_err("failed to create device file(%s)!\n",
						dev_attr_regs.attr.name);
		goto err_regs;
	}
						...
						const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("Fail to read STMPE811 ID\n");
		adc->id = 0;
