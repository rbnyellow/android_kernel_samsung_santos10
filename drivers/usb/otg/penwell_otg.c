...
#ifdef CONFIG_USB_PENWELL_OTG_SAMSUNG
#include <linux/extcon.h>
#endif

/* Eyediagram optimize value */
#define ULPI_VS1SET_OPTIMIZE	0x77
#define ULPI_VS3_OPTIMIZE		0x05
...
#define readl(addr) ({ if (check_pm_otg()) { \
	panic("usb otg, read reg:0x%p, pm_sss0_base:0x%x",  \
	addr, *(pm_sss0_base)); }; __le32_to_cpu(__raw_readl(addr)); })
#define writel(b, addr) ({ if (check_pm_otg()) { \
	panic("usb otg, write reg:0x%p, pm_sss0_base:0x%x",  \
...
#ifndef CONFIG_USB_PENWELL_OTG_SAMSUNG
	if (val32 & OTGSC_BSV)
...
static void penwell_otg_eye_diagram_optimize(void)
{
	...
	if (is_clovertrail(to_pci_dev(pnw->dev))) {
		...
		value = ULPI_VS1SET_OPTIMIZE;
	}

	retval = penwell_otg_ulpi_write(iotg, ULPI_VS1SET, value);
	if (retval)
		dev_warn(pnw->dev, "eye diagram optimize failed with ulpi failure : ULPI_VS1SET\n");

	retval = penwell_otg_ulpi_read(iotg, ULPI_VS3, &value);
	if (retval)
		dev_warn(pnw->dev, "eye diagram optimize failed with ulpi failure : ULPI_VS3 read\n");

	value &= ~0x07;
	value |= ULPI_VS3_OPTIMIZE;
	retval = penwell_otg_ulpi_write(iotg, ULPI_VS3, value);

	if (retval)
		dev_warn(pnw->dev, "eye diagram optimize failed with ulpi failure : ULPI_VS3 write\n");
...
static void penwell_vbus_session_valid(struct penwell_otg *pnw, int enable)
{
	pr_info("%s enable: %d\n", __func__, enable);
	if (enable) {
		pnw->iotg.hsm.b_sess_vld = 1;
		pnw->iotg.otg.state = OTG_STATE_B_IDLE;
	} else
		pnw->iotg.hsm.b_sess_vld = 0;

	if (pnw->dev->power.is_suspended)
		pr_info("%s device wasn't resumed yet\n", __func__);
	else
		penwell_update_transceiver();
}
...
static int __init sec_penwell_init_cable_notify(void)
{
	struct sec_cable *cable;
	int i;
	int ret;
...
device_initcall_sync(sec_penwell_init_cable_notify);
#endif
...
#ifdef CONFIG_USB_PENWELL_OTG_SAMSUNG
	val32 = OTGSC_DPIE | OTGSC_BSEIE | OTGSC_ASVIE |
		OTGSC_AVVIE | OTGSC_IDIE | OTGSC_IDPU;
#else
	/* enable OTGSC int */
