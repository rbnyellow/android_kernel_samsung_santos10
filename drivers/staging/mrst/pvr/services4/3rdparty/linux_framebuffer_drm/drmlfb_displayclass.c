			 ...
			 MRST_BOOL bFlip)
{
	...
	for(i = 0; i < psSwapChain->ulSwapChainLength; i++)
	{
		...
		DRM_INFO(" FlushInternalVSyncQueue: Flushing swap buffer (index %lu)\n", psSwapChain->ulRemoveIndex);
		...
		{
			DRM_INFO(" FlushInternalVSyncQueue: Calling command complete for swap buffer (index %lu)\n", psSwapChain->ulRemoveIndex);
...
static IMG_VOID SetDCState(IMG_HANDLE hDevice, IMG_UINT32 ui32State)
{
	...
	switch (ui32State)
	{
		...
		case DC_STATE_FORCE_SWAP_TO_SYSTEM:
			DRM_INFO("FORCE SWAP TO SYSTEM");
			SetFlushState(psDevInfo, MRST_TRUE);/*this blocks until all buffer on display queue is flushed*/
			SetFlushState(psDevInfo, MRST_FALSE);
			break;
		default:
			break;
	...
	IMG_HANDLE hSwapChain)
{
	...
	if (psDevInfo->ui32SwapChainNum == 0)
	{
		 ...
		 * unmapped, overlay will be abnormal, here need disable overlay.
		 */
			...
			IMG_VOID *pvData)
{
...
#if 0
	/* Intel graphic engineer(Liu Hong) recommends to skip below exit_idle()
	...
	if (dsi_config->type == MDFLD_DSI_ENCODER_DBI) {
		...
		while (loop_poll && !DRMLFBFifoEmpty(psDevInfo) && retry) {
