...
#define MIN_FREESWAP_PAGES 		(NR_TO_RECLAIM_PAGES*2*NR_CPUS) /* 4MB*cpu_core*2 */
...
extern long rtcc_reclaim_pages(long nr_to_reclaim, long swap_en);

static long care_cache(void)
{
    long free, file, swap_en = 1;

    free = global_page_state(NR_FREE_PAGES);
    file = global_page_state(NR_FILE_PAGES) - total_swapcache_pages;

    if (file > lowmem_minfree[lowmem_minfree_size-1])
        swap_en = 0;
...
static int do_compcache(void * nothing)
{
	...
	for ( ; ; ) {
		...
		if (rtcc_reclaim_pages(number_of_reclaim_pages, care_cache()) < minimum_reclaim_pages)
			cancel_soft_reclaim();
