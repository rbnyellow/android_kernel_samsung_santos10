...
int psb_set_brightness(struct backlight_device *bd)
{
	...
	if (level > 0) {
		if (pdata->is_need_pwron_bl()) {
			pdata->backlight_power(true, DEFAULT);
			mdelay(1);
		}
		...
		cmc624_set_pwm(level);
		pdata->backlight_power(false, DEFAULT);
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cmc624_set_auto_brightness(value);
...
int psb_backlight_sec_init(void)
{
#ifdef CONFIG_BACKLIGHT_CLASS_DEVICE
...
#if 0 //Workaround to avoid duplication issue.
