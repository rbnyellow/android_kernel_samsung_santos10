...
static int __init sec_cmc624_set_board_rev(char *str)
{
	unsigned int rev;
...
early_param("androidboot.revision", sec_cmc624_set_board_rev);
...
static int cmc624_enable(void)
{
	...
	if (!pdata->skip_init) {
		if (g_cmc624_info->pdata->board_rev >= 11)
			udelay(20);
		/* FAIL_SAFEB HIGH */
				...
				const struct i2c_device_id *id)
{
	...
	pr_info("sec_board_rev : %d\n", g_cmc624_info->pdata->board_rev);

	/* register tune data list */
	ret = santos10_init_tune_list();
	if (ret < 0) {
...
// move to panel-tc35876x-vid.c
//MODULE_DEVICE_TABLE(i2c, sec_cmc624_ids);

static struct i2c_driver sec_cmc624_i2c_driver = {
	.driver	= {
		.name	= "sec_cmc624_i2c",
		.owner = THIS_MODULE,
	},
...
int fcall_cmc624_init(void)
{
	return i2c_add_driver(&sec_cmc624_i2c_driver);
}

void fcall_cmc624_exit(void)
{
	i2c_del_driver(&sec_cmc624_i2c_driver);
}

#if 0 // Workaround to avoid mutiple definiton.. 
