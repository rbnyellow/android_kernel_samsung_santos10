					...
					struct cpuidle_device *dev, int index)
{
	...
	if (xen_start_info) {
		if (!need_resched())
			HYPERVISOR_mwait_idle_op(eax, 1, (void *)&current_thread_info()->flags,
					__pa(&current_thread_info()->flags));
	} else
#endif
	{
		...
		struct cpuidle_driver *drv, int index)
{
	...
	if (xen_start_info) {
		if (!need_resched())
			HYPERVISOR_mwait_idle_op(eax, ecx, (void *)&current_thread_info()->flags,
					__pa(&current_thread_info()->flags));
	} else
#endif
	{
