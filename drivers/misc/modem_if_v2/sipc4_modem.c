			...
			struct modem_data *pdata)
{
	...
	iod->mc = modemctl;
	if (iod->format == IPC_FMT && iod->id == 0x1)
		modemctl->iod = iod;
	else if (iod->format == IPC_BOOT && iod->id == 0)
		modemctl->bootd = iod;
	...
	if (iod->format != IPC_RAW)
		sipc4_insert_iod_with_format(msd, iod->format, iod);

	if (sipc4_is_not_reserved_channel(iod->id))
		sipc4_insert_iod_with_channel(msd, iod->id, iod);

	/* register misc device or net device */
	ret = _sipc4_init_io_device(iod);
	if (ret) {
		kfree(iod);
