...
static struct hsi_channel_info ch_info[HSI_NUM_OF_USE_CHANNELS] = {
	...
	{ "dbg", 0, HSI_RX_CPDEBUG_CHAN_PDU,
				HSI_TX_CPDEBUG_CHAN_PDU, 0, 0 },
	{ "extra", 0, HSI_RX_EXTRA_CHAN_PDU,
				HSI_TX_EXTRA_CHAN_PDU, 0, 0 },
};
...
check_online:
	...
	if (skb_queue_len(tx_q)) {
		...
		if (channel->channel_id == HSI_CONTROL_CHANNEL)
			tx_msg->complete = if_hsi_ctrl_write_done;
		else
		...
		*hd_ptr32 = HSI_EDLP_PDU_SIGNATURE;
		...
		if (down_timeout(&channel->write_done_sem,
			...
			if ((mipi_ld->ld.com_state == COM_ONLINE) &&
				(mipi_ld->ld.mc->phone_state == STATE_ONLINE) &&
				(mipi_ld->ld.mc->gpio_ap_dump_int))
...
retry_open:
	...
	if (down_timeout(&channel->open_conn_done_sem,
		...
		mipi_err("(%d)ack timeout for open command, cnt=%d\n", ch_num,
					retry_cnt);
		retry_cnt++;
		if (retry_cnt < 5)
			goto retry_open;
...
static void if_hsi_start_work(struct work_struct *work)
{
	...
	ret = if_hsi_open_channel(mipi_ld, HSI_MULTI_FMT_CHANNEL);
	if (ret < 0) {
		mipi_err("multi-fmt(5) channel open fail: %d\n", ret);
		schedule_delayed_work(&mipi_ld->start_work, 3 * HZ);
		return;
	} else
		mipi_info("multi-fmt(5) channel open Done\n");

	ret = if_hsi_open_channel(mipi_ld, HSI_MODEM_DEBUG_CHANNEL);
	if (ret < 0)
		mipi_err("debug(6) channel open fail: %d\n", ret);
	else
		mipi_info("debug(6) channel open Done\n");

	ret = if_hsi_open_channel(mipi_ld, HSI_EXTRA_CHANNEL);
	if (ret < 0)
		mipi_err("extra(7) channel open fail: %d\n", ret);
	else
		mipi_info("extra(7) channel open Done\n");
...
static int if_hsi_init_handshake(struct mipi_link_device *mipi_ld, int mode)
{
	...
	case HSI_INIT_MODE_NORMAL:
		...
		for (i = 0; i < HSI_NUM_OF_USE_CHANNELS; i++) {
			...
			if (mipi_ld->hsi_channels[i].use_credits)
				atomic_set(&mipi_ld->hsi_channels[i].credits,
							0);
		}
...
{
	u32 cmd = cmd_type_to_cmd(cmd_type);
	...
	case HSI_EDLP_MSG_CONF_CH:
		cmd |= ch_to_cmd(ch) | param; /* config data */
		return cmd;

	case HSI_EDLP_MSG_OPEN_CONN:
		cmd |= ch_to_cmd(ch) | param / 4; /* PDU size in 32bit word */
		return cmd;

	case HSI_EDLP_MSG_CLOSE_CONN:
		cmd |= ch_to_cmd(ch) | direction_to_cmd(param); /* Direction */
		return cmd;

	case HSI_EDLP_MSG_ACK:
		cmd |= ch_to_cmd(ch) | param; /* Cmd ID & Echoed parameter */
		return cmd;
	...
	case HSI_EDLP_MSG_CREDITS:
		cmd |= ch_to_cmd(ch) | param; /* Credits */
		return cmd;
...
{
	int ret;
	u32 ch, param, pdu_size;
	u32 cmd_type = cmd_to_cmd_type(cmd);
	unsigned long int flags;
	u32 credit_cnt = 0;
	int total_credit = 0;

	mipi_debug("if_hsi_rx_cmd_handle cmd=0x%x, type=0x%x\n", cmd, cmd_type);
	...
	case HSI_EDLP_MSG_ECHO:
		...
		if (cmd_to_echo_param(cmd) != HSI_EDLP_MSG_ECHO_CHECK_PATTERN) {
	...
	case HSI_EDLP_MSG_NOP:
		if (cmd_to_nop_param(cmd) != HSI_EDLP_MSG_NOP_CHECK_PATTERN) {
	...
	case HSI_EDLP_MSG_CONF_CH:
		ch = cmd_to_ch(cmd);
		param = cmd_to_config_data(cmd);
		mipi_info("Receive CONF_CH ch:%d, 0x%x\n", ch, param);

		/* No Use */
		param |= cmd_id_to_param(HSI_EDLP_MSG_CONF_CH);
	...
	case HSI_EDLP_MSG_CLOSE_CONN:
		ch = cmd_to_ch(cmd);
		param = cmd_to_direction(cmd);
		mipi_info("Receive CLOSE_CONN ch:%d, 0x%x\n", ch, param);

		param |= cmd_id_to_param(HSI_EDLP_MSG_CLOSE_CONN);
		ret = if_hsi_send_command(mipi_ld, HSI_EDLP_MSG_ACK,
					ch, param);
		if (ret) {
			mipi_err("if_hsi_send_command fail=%d\n", ret);
	...
	case HSI_EDLP_MSG_ACK:
		ch = cmd_to_ch(cmd);
		param = cmd_to_ack_param(cmd);
		mipi_debug("Receive ACK ch:%d, 0x%x\n", ch, param);

		switch (param_to_cmd_id(param)) {
		case HSI_EDLP_MSG_CONF_CH:
			mipi_debug("got CONF_CH ACK\n");
			up(&mipi_ld->hsi_channels[ch].conf_ch_done_sem);
			break;
		case HSI_EDLP_MSG_OPEN_CONN:
			mipi_debug("got OPEN_CONN ACK\n");
			up(&mipi_ld->hsi_channels[ch].open_conn_done_sem);
			break;
		case HSI_EDLP_MSG_CLOSE_CONN:
			mipi_debug("got CLOSE_CONN ACK\n");
			up(&mipi_ld->hsi_channels[ch].close_conn_done_sem);
			break;
		case HSI_EDLP_MSG_CREDITS:
			mipi_debug("got CREDITS ACK\n");
			up(&mipi_ld->hsi_channels[ch].credits_done_sem);
			break;
		default:
			mipi_err("wrong cmd_id:%d\n", param_to_cmd_id(param));
			return -1;
		}
		break;
	...
	case HSI_EDLP_MSG_CREDITS:
		ch = cmd_to_ch(cmd);
		credit_cnt = cmd_to_credits(cmd);
		mipi_debug("Receive CREDITS ch:%d, 0x%x\n", ch, credit_cnt);

		if (atomic_read(&mipi_ld->hsi_channels[ch].credits)) {
			spin_lock_irqsave(&mipi_ld->credit_lock, flags);
			total_credit = atomic_add_return(credit_cnt,
					&mipi_ld->hsi_channels[ch].credits);
			spin_unlock_irqrestore(&mipi_ld->credit_lock, flags);
			mipi_debug("total CREDITS ch:%d, %d(+%d)\n", ch,
						total_credit, credit_cnt);
		}

		param = credit_cnt;
		param |= cmd_id_to_param(HSI_EDLP_MSG_CREDITS);
						...
						struct if_hsi_channel *channel)
{
	...
	int ret;
	u32 cmd;
	int one_pkt_sz = 0, rest_pkt_sz = 0, read_pkt_sz = 0;
	...
	case COM_ONLINE:
		...
		cmd = *(u32 *)channel->rx_data;
		mipi_debug("rx_cmd : %08x\n", cmd);
		ret = if_hsi_rx_cmd_handle(mipi_ld, cmd);
		if (ret)
	...
	case COM_BOOT_EBL:
		...
		if (iod) {
			skb = alloc_skb(channel->rx_count, GFP_ATOMIC);
			if (!skb) {
	...
	case COM_CRASH:
		...
		if (iod) {
			...
			mipi_debug("ramdump packet size : %d\n",
						channel->packet_size);
			rest_pkt_sz = channel->packet_size;
			read_pkt_sz = 0;

send_skb_rest_crash:

			one_pkt_sz = min(HSI_RX_SKB_MAX_LENGTH, rest_pkt_sz);
			rest_pkt_sz -= one_pkt_sz;
			skb = alloc_skb(one_pkt_sz, GFP_ATOMIC);
			if (!skb) {
				mif_err("fail alloc skb (%d)\n", __LINE__);
				return;
			}
			memcpy(skb_put(skb, one_pkt_sz),
				(void *)(channel->rx_data + 4 + read_pkt_sz),
				one_pkt_sz);
			read_pkt_sz += one_pkt_sz;
			ret = iod->recv_skb(iod, &mipi_ld->ld, skb);
			if (ret < 0)
				mipi_err("recv call fail : %d\n", ret);

			mipi_debug("read_pkt_sz: %d(%d)\n",
						read_pkt_sz, rest_pkt_sz);
			if (rest_pkt_sz)
				goto send_skb_rest_crash;
		}
	...
	struct mipi_link_device *mipi_ld, unsigned int ch, int actual_len)
{
	...
	int force_pad = channel->is_dma_ch ? HSI_NET_PAD_FOR_CP : 0;
	int rest_pkt_sz, read_pkt_sz, one_pkt_sz;
	...
	do {
		...
		*/
		...
		rest_pkt_sz = pk_sz - force_pad;
		read_pkt_sz = 0;
		mipi_debug("total pkt_sz: %d\n", rest_pkt_sz);
...
send_skb_rest:
		...
		if (iod->ndev) {
			...
			if (skb) {
				...
				one_pkt_sz = pk_sz - force_pad;
				rest_pkt_sz = 0;
			}
		} else {
			one_pkt_sz = min(HSI_RX_SKB_MAX_LENGTH, rest_pkt_sz);
			rest_pkt_sz -= one_pkt_sz;
			mipi_debug("one_pkt_sz: %d, rest_pkt_sz: %d\n",
						one_pkt_sz, rest_pkt_sz);
		...
		memcpy(skb_put(skb, one_pkt_sz),
		(void *)(buff + pk_offset + force_pad + read_pkt_sz),
					one_pkt_sz);
		read_pkt_sz += one_pkt_sz;
...
static void if_hsi_ctrl_write_done(struct hsi_msg *msg)
{
	unsigned long int flags;
	struct mipi_link_device *mipi_ld =
			(struct mipi_link_device *)hsi_client_drvdata(msg->cl);
	...
	/* PATCH : additional protocol for credits usage
	 *
	 * do not start to send tx data until MSG_ACK for CREDITS is recieved
	 */
	cmd = *(u32 *)sg_virt(msg->sgt.sgl);
	if (cmd_to_cmd_type(cmd) == HSI_EDLP_MSG_ACK &&
				param_to_cmd_id(cmd) == HSI_EDLP_MSG_CREDITS) {
		ch = cmd_to_ch(cmd);
		credit_cnt = cmd_ack_to_credits(cmd);
		mipi_debug("send credit-ack done: ch=%d, 0x%x, add_credit=%d\n",
							ch, cmd, credit_cnt);
...
out:
	mipi_ld->hsi_channels[msg->channel].tx_count = msg->actual_len;
	up(&mipi_ld->hsi_channels[msg->channel].write_done_sem);
...
static void if_hsi_ctrl_read_done(struct hsi_msg *msg)
{
	int ret;
	struct mipi_link_device *mipi_ld =
			(struct mipi_link_device *)hsi_client_drvdata(msg->cl);
			...
			unsigned int ch_num, unsigned int size)
{
	...
	if (ch_num == HSI_CONTROL_CHANNEL)
		rx_msg->complete = if_hsi_ctrl_read_done;
	else
...
static void if_hsi_shutdown(struct device *dev)
{
	...
	if (hsi_port_claimed(mipi_ld->client)) {
		...
		hsi_flush(mipi_ld->client);
		hsi_unregister_port_event(mipi_ld->client);
		hsi_release_port(mipi_ld->client);
	}
