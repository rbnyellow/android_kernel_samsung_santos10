...
multifmt:
	...
	if (iod->id == SIPC5_CH_ID_FMT_0)
		print_hex_dump(KERN_DEBUG, "FMT-TX: ",
				...
				(size_t)skb->len :
				(size_t)16, false);
	else if (iod->id == SIPC5_CH_ID_FMT_9)
		print_hex_dump(KERN_DEBUG, "LOOP-TX: ",
				DUMP_PREFIX_NONE,
				1, 1,
				(void *)skb->data,
				skb->len <= 16 ?
				(size_t)skb->len :
				(size_t)16, false);
	/* else if (iod->id == SIPC5_CH_ID_RFS_0)
		print_hex_dump(KERN_DEBUG, "RFS-TX: ",
				DUMP_PREFIX_NONE,
				1, 1,
				(void *)skb->data,
				skb->len <= 16 ?
...
continue_multiframe:
	...
	if (iod->id == SIPC5_CH_ID_FMT_0)
		...
		print_hex_dump(KERN_DEBUG, "LOOP-RX: ",
				DUMP_PREFIX_NONE,
				1, 1,
				(void *)skb->data,
				len <= 16 ?
				...
				(size_t)16, false);
	/* else if (iod->id == SIPC5_CH_ID_RFS_0)
		print_hex_dump(KERN_DEBUG, "RFS-RX: ",
				DUMP_PREFIX_NONE,
				1, 1,
				(void *)skb->data,
				len <= 16 ?
