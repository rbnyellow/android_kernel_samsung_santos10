...
#define HSI_WRITE_DONE_TIMEOUT	(HZ / 2)
#define HSI_READ_DONE_TIMEOUT	(HZ)
#define HSI_ACK_DONE_TIMEOUT	(HZ / 2)
#define HSI_ACWAKE_DOWN_TIMEOUT	(HZ / 2) /* 500 ms */
...
#define HSI_TX_CPDEBUG_CHAN_PDU	(1 * 1024)
#define HSI_TX_EXTRA_CHAN_PDU	(6 * 1024 + 512)

#define HSI_RX_SKB_MAX_LENGTH	(4 * 1024 - 512)
...
#define cmd_type_to_cmd(x)	(((x) & 0x0000000F) << 28)
#define ch_to_cmd(x)		(((x) & 0x0000000F) << 24)
#define direction_to_cmd(x)	(((x) & 0x000000FF) << 16)
#define credits_to_cmd(x)	(((x) & 0x000000FF) << 16)

#define cmd_to_cmd_type(x)	(((x) & 0xF0000000) >> 28)
#define cmd_to_echo_param(x)	(((x) & 0x00FFFFFF))
#define cmd_to_nop_param(x)	(((x) & 0x00FFFFFF))
#define cmd_to_ch(x)		(((x) & 0x0F000000) >> 24)
#define cmd_to_config_data(x)	(((x) & 0x00FFFF00) >> 8)
#define cmd_to_pdu_len(x)	(((x) & 0x000FFFFF))
#define cmd_to_direction(x)	(((x) & 0x00FF0000) >> 16)
#define cmd_to_credits(x)	(((x) & 0x00FF0000) >> 16)
#define cmd_ack_to_credits(x)	(((x) & 0x000000FF))
#define cmd_to_ack_param(x)	(((x) & 0x00FFFFFF))
#define cmd_to_nak_param(x)	(((x) & 0x00FFFFFF))

#define param_to_cmd_id(x)	(((x) & 0x00F00000) >> 20)
#define cmd_id_to_param(x)	(((x) & 0x0000000F) << 20)
