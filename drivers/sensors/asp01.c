...
static int asp01_load_caldata(struct asp01_data *data)
{
	struct file *cal_filp = NULL;
	mm_segment_t old_fs;
	int err = -1, i;
...
static void asp01_open_calibration(struct asp01_data *data)
{
	...
	if (data->cal_data_loaded == false) {
		err = asp01_load_caldata(data);
		if (err < 0) {
			pr_info("%s, load_caldata failed\n", __func__);
			return;
		}
		data->cal_data_loaded = true;
	} else
		pr_info("%s, cal data is already loaded\n", __func__);
				    ...
				    const char *buf, size_t count)
{
	...
	pr_info("%s, enable = %ld, shutdown = %d\n", __func__,
		enable, data->shutdown);
	if (data->shutdown)
		return -ENXIO;
	...
	if (enable && !atomic_read(&data->enable)) {
		...
		/* disabled with close. */
		if (!gpio_get_value(data->pdata->t_out))
			data->init_touch_needed = true;
...
static void asp01_work_func(struct work_struct *work)
{
	...
	if (!jig_on && !data->skip_data) {
		...
		pr_info("%s: skip input report jig_on: %d, skip cmd: %d\n",
					...
					char *buf)
{
	...
	if (asp01_load_caldata(data) < 0)
		pr_err("%s, load caldata failed\n", __func__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	u16 cs_per[4] = {0,};
	u16 cr_per[4] = {0,};
	u16 cs_per_sort[4], cr_per_sort[4];
	...
	for (i = 0; i < 4; i++) {
		...
		cs_per_sort[i] = cs_per[i] = (tmp_data[1] << 8) | tmp_data[0];
		...
		cr_per_sort[i] = cr_per[i] = (tmp_data[1] << 8) | tmp_data[0];
	}
	...
	for (i = 0; i < 2; i++) {
		for (j = i + 1; j < 4; j++) {
			if (cs_per_sort[i] > cs_per_sort[j])
				swap(cs_per_sort[i], cs_per_sort[j]);
			if (cr_per_sort[i] > cr_per_sort[j])
				swap(cr_per_sort[i], cr_per_sort[j]);
		}
	}

	return sprintf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",\
		cs_per[0], cs_per[1], cs_per[2], cs_per[3],\
		cs_per_sort[1],\
		cr_per[0], cr_per[1], cr_per[2], cr_per[3],\
		cr_per_sort[1]);
}
		       ...
		       const struct i2c_device_id *id)
{
	...
	if (pdata) {
		data->pdata = pdata;
		err = data->pdata->request_gpio();
		if (err < 0) {
			pr_err("%s, Setting grip sensor gpio failed(%d)\n",
				__func__, err);
			goto err_read_reg;
		}
	} else {
