...
struct bh1730fvc_data {
	int (*reset)(void);
	struct bh1730fvc_platform_data *pdata;
...
static int bh1730fvc_test_device(struct bh1730fvc_data *bh1730fvc);
...
static int bh1730fvc_write_byte(struct i2c_client *client, u8 reg, u8 value)
{
	...
	do {
		...
		retry++;
		pr_err("%s, i2c_master_send retry=%d\n",
			__func__, retry);
		usleep_range(10000, 11000);
	} while (retry < 2);

	if (err != 2) {
		pr_err("%s: i2c_master_send fail cmd=0x%x, err=%d\n",
			__func__, cmd[1], err);
		err = -EIO;
	} else
		err = 0;
...
static int bh1730fvc_disable(struct bh1730fvc_data *bh1730fvc)
{
	...
	if (unlikely(err < 0))
		pr_err("%s: Failed to write byte (POWER_OFF) %d\n",
			__func__, err);
...
static void bh1730fvc_work_func_light(struct work_struct *work)
{
	...
	if (count++ == 100) {
		pr_info("%s, %d,%d,%d,%d\n", __func__,
			bh1730fvc->data0 + 1, bh1730fvc->data1 + 1,
			bh1730fvc->gain, bh1730fvc->time);
		count = 0;
	}
}
...
int bh1730fvc_test_device(struct bh1730fvc_data *bh1730fvc)
{
	...
	pr_err("%s : slave addr : 0x%x\n", __func__, bh1730fvc->client->addr);
					...
					const struct i2c_device_id *id)
{
	...
	struct input_dev *input_dev;
	struct bh1730fvc_platform_data *pdata = client->dev.platform_data;
...
void bh1730fvc_shutdown(struct i2c_client *client)
{
	struct bh1730fvc_data *bh1730fvc = i2c_get_clientdata(client);
...
static int bh1730fvc_suspend(struct device *dev)
{
	...
	pr_info("%s, state = %d\n", __func__, bh1730fvc->state);
	if (bh1730fvc->state) {
		err = bh1730fvc_disable(bh1730fvc);
		if (err < 0)
			pr_err("%s: could not disable(%d)\n", __func__, err);
	}
