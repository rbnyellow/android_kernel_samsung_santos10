...
static int fg_read_temp(struct i2c_client *client)
{
	...
	/* There was sudden increase in temperature once temperature reaches
	59 c (from 59c to 69 c)hence adjusting the temperature here and this increase
	was consistent after it reaches 59c hence adjustment*/

	if(temper >= 60000)
		return ((temper - 10000)/100);
...
static int fg_read_current(struct i2c_client *client, int unit)
{
	...
	if ((fuelgauge->info.pr_cnt++ % PRINT_COUNT) == 18) {
		...
		read_val = fg_read_register(client, QRTable30_REG);
		if (read_val != 0x1004) {
			dev_info(&client->dev,
				"%s: QRTable30_REG data corruption, rewrite!!\n",
				__func__);
			fg_write_register(client, QRTable30_REG, (u16)0x1004);
		}
	}
...
void fg_low_batt_compensation(struct i2c_client *client, u32 level)
{
	...
	/* [1% compensation] RemCapREP (05h) = FullCap(10h) x 0.0090 */
				...
				struct i2c_client *client, int *nLevel)
{
	...
	if (fuelgauge->info.low_batt_comp_count > MAX_LOW_BATT_CHECK_CNT) {
		dev_info(&client->dev,
			"%s : over MAX_LOW_BATT_CHECK_CNT\n", __func__);
		*nLevel = 1;
		ret = 1;
	}
		...
		for (i = 1; i < CURRENT_RANGE_MAX_NUM; i++) {
			if ((fg_min_current >= get_battery_data(fuelgauge).
				...
				if (fg_soc >= 10 && fg_vcell <
					...
					i, fg_min_current, 1)) {
					fuelgauge->info.low_batt_comp_count++;
					dev_info(&client->dev,
						"%s:low_batt_comp_count(%d)\n",
						__func__,
						fuelgauge->info.low_batt_comp_count);
				} else {
					fuelgauge->info.low_batt_comp_count = 0;
				}
			}
		}
...
bool sec_hal_fg_fuelalert_process(void *irq_data, bool is_fuel_alerted)
{
	...
	/* skip fuelalert_process, when Fuel-alert NOT alerted */
	if (!is_fuel_alerted)
		return true;

	if ((fuelgauge->info.soc - current_soc) <= STABLE_LOW_BATTERY_DIFF)
