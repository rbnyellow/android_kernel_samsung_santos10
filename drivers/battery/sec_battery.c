...
static int __init sec_bat_set_board_rev(char *str)
{
	unsigned int rev;
...
early_param("androidboot.revision", sec_bat_set_board_rev);
				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_THERMAL_SOURCE_FG:
		...
		/* read wrong temp value, in HW_REV03 */
		if (sec_board_rev == 3) {
			battery->temperature = 200;
			pr_info("%s: set as 20, in sec_board_rev(%d)\n",
				__func__, sec_board_rev);
		} else
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if ((current_cable_type >= 0) &&
			...
			/* if power sharing cable is connected,
			  * skip the cable work
			  */
			if (current_cable_type ==
				...
				queue_work(battery->monitor_wqueue,
						&battery->monitor_work);
			} else if (battery->cable_type != current_cable_type) {
	...
	case POWER_SUPPLY_PROP_ONLINE:
		if (battery->ps_status) {
			...
			if (battery->ps_enable == true) {
				...
				dev_info(battery->dev,
				"%s: power sharing cable disconnected! (%d)\n",
					__func__, battery->ps_enable);
