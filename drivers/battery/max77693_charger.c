...
static void max77693_dump_reg(struct max77693_charger_data *charger)
{
	...
	for (reg_addr = 0xB0; reg_addr <= 0xC5; reg_addr++) {
		max77693_read_reg(charger->max77693->regmap, reg_addr,
				&reg_data);
...
static bool max77693_charger_unlock(struct max77693_charger_data *chg_data)
{
	...
	do {
		max77693_read_reg(chg_data->max77693->regmap,
		...
		if (chgprot != 0x03) {
			...
			max77693_write_reg(chg_data->max77693->regmap,
					MAX77693_CHG_REG_CHG_CNFG_06,
					(0x03 << 2));
			need_init = true;
			msleep(20);
...
static int max77693_get_battery_present(struct max77693_charger_data *charger)
{
	...
	if (max77693_read_reg(charger->max77693->regmap,
		...
		int enable)
{
	...
	max77693_read_reg(charger->max77693->regmap,
	...
	max77693_write_reg(charger->max77693->regmap,
			MAX77693_CHG_REG_CHG_CNFG_00, reg_data);
}
		...
		int cur)
{
	...
	if (reg_data == set_current_reg) {
		/* check uvlo  */
		while ((set_current_reg > (MINIMUM_INPUT_CURRENT / 20)) &&
				(set_current_reg < 255)) {
...
static void max77693_recovery_work(struct work_struct *work)
{
	...
	if ((!chg_data->is_charging) || mutex_is_locked(&chg_data->ops_lock) ||
		...
		return;
	max77693_read_reg(chg_data->max77693->regmap,
				MAX77693_CHG_REG_CHG_DETAILS_00, &dtls_00);
	max77693_read_reg(chg_data->max77693->regmap,
				MAX77693_CHG_REG_CHG_DETAILS_01, &dtls_01);
	max77693_read_reg(chg_data->max77693->regmap,
				MAX77693_CHG_REG_CHG_DETAILS_02, &dtls_02);
	...
	if ((chg_data->soft_reg_recovery_cnt < RECOVERY_CNT) && (
		...
		if (chg_data->soft_reg_recovery_cnt < RECOVERY_CNT) {
			...
			queue_delayed_work(chg_data->wqueue,
				&chg_data->recovery_work,
...
static int max77693_get_vbus_state(struct max77693_charger_data *charger)
{
	...
	max77693_read_reg(charger->max77693->regmap,
		MAX77693_CHG_REG_CHG_DETAILS_00, &reg_data);
#if defined(CONFIG_CHARGER_MAX77803)
	if (charger->cable_type == POWER_SUPPLY_TYPE_WIRELESS)
...
static int max77693_get_charger_state(struct max77693_charger_data *charger)
{
	...
	max77693_read_reg(charger->max77693->regmap,
		MAX77693_CHG_REG_CHG_DETAILS_01, &reg_data);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = max77693_get_input_current(charger) * 1000;
		break;
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if ((charger->status == POWER_SUPPLY_STATUS_CHARGING) ||
			...
			max77693_set_charge_current(charger,
				set_charging_current);
			/* if battery is removed, disable input current and
			 * reenable input current to enable buck always
			 */
...
static void max77693_chgin_isr_work(struct work_struct *work)
{
	...
	while (1) {
		...
		chgin_dtls = ((chgin_dtls & MAX77693_CHGIN_DTLS) >>
				MAX77693_CHGIN_DTLS_SHIFT);
		max77693_read_reg(charger->max77693->regmap,
				MAX77693_CHG_REG_CHG_DETAILS_01, &chg_dtls);
...
static void max77693_chgin_init_work(struct work_struct *work)
{
	...
	ret = request_threaded_irq(charger->irq_chgin, NULL,
			max77693_chgin_irq, IRQF_NO_SUSPEND,
			"chgin-irq", charger);
	if (ret < 0) {
...
static __devinit int max77693_charger_probe(struct platform_device *pdev)
{
	...
	/* using irq_create_mapping for chgin-irq */
	virq = irq_create_mapping(charger->max77693->irq_domain,
			MAX77693_CHG_IRQ_CHGIN_I + charger->max77693->irq_base);
	if (!virq)
		pr_err("%s: failed to create chgin-irq mapping\n", __func__);
	charger->irq_chgin = virq;

	/* enable chgin irq after sec_battery_probe */
	queue_delayed_work(charger->wqueue, &charger->chgin_init_work,
			msecs_to_jiffies(3000));

	/* using irq_create_mapping for bypass-irq */
	virq = irq_create_mapping(charger->max77693->irq_domain,
			MAX77693_CHG_IRQ_BYP_I + charger->max77693->irq_base);
	if (!virq)
		pr_err("%s: failed to create bypass-irq mapping\n", __func__);
	charger->irq_bypass = virq;

	ret = request_threaded_irq(charger->irq_bypass, NULL,
			max77693_bypass_irq, IRQF_NO_SUSPEND,
			"bypass-irq", charger);
	if (ret < 0)
...
void max77693_charger_shutdown(struct device *dev)
{
	...
	if (!charger->max77693->regmap) {
		pr_err("%s: no max77693 regmap\n", __func__);
		return;
	}
	reg_data = 0x04;
	max77693_write_reg(charger->max77693->regmap,
