...
{
	if (data->atch.enable) {
		mxt_write_object(data, MXT_PROCI_EXTRATOUCHSCREENDATA_T57, 1, 10);
		/* Fast Drift */
						...
						struct mxt_message *message)
{
	...
	if (data->atch.coin) { /* 1st Stage Check */
		__mxt_debug_msg(data,
				"ATCH: COIN TCHAREA=%d ATCHAREA=%d SUM=%d TCNT=%d\n",
				tch_area, atch_area, sum_size, touch_cnt);
		if (touch_cnt) {
			if (data->atch.timer == 0) {
				...
				data->atch.timer = 1;
				/* Battery Mode Overflow Recovery */
			...
			if (atch_area > 4) { /* Only Anti-touch */
				__mxt_debug_msg(data, "ATCH: CASE 1-1\n");
				mxt_check_command_calibration(data);
			}
		}
		case1_cnt = 0, case2_cnt = 0, case3_cnt = 0;
		abn_case1 = 0, abn_case2 = 0, abn_case3 = 0;
		abn_case4 = 0, abn_case5 = 0, abn_case6 = 0;
		abn_case7 = 0, abn_case8 = 0;
	}

	if (data->atch.calgood && touch_cnt) { /* 2nd Stage Check */
		__mxt_debug_msg(data,
				"ATCH: CALGOOD TCHAREA=%d ATCHAREA=%d SUM=%d TCNT=%d\n",
				tch_area, atch_area, sum_size, touch_cnt);
		if (tch_area) {
			...
			if (data->t9_size == 0 && data->t9_amp > 10 && atch_area > 30 &&
				touch_cnt < 6 && (case2_cnt++ > 4)) {
				__mxt_debug_msg(data, "ATCH: CASE 2-6\n");
				mxt_check_command_calibration(data);
			}
			if (sum_size-tch_area > 90 && touch_cnt < 3) {
				__mxt_debug_msg(data, "ATCH: CASE 2-7\n");
				mxt_check_command_calibration(data);
			}
			if (data->atch.prev_chargin_status == 0) {
				if (data->t9_size < 16 && data->t9_amp > 30 &&
					sum_size > 30 && touch_cnt < 7) {
					__mxt_debug_msg(data, "ATCH: CASE 2-8\n");
					mxt_check_command_calibration(data);
				}
				if (data->t9_size < 10 && data->t9_amp > 55 &&
					sum_size < 40 && (case3_cnt++ > 3)) {
					__mxt_debug_msg(data, "ATCH: CASE 2-9\n");
					mxt_check_command_calibration(data);
				}
			}
		} else if (atch_area) { /* Only Anti-touch */
			__mxt_debug_msg(data, "ATCH: CASE 2-0\n");
			mxt_check_command_calibration(data);
		}
		abn_case1 = 0, abn_case2 = 0, abn_case3 = 0;
		abn_case4 = 0, abn_case5 = 0, abn_case6 = 0;
	...
	if (data->atch.abnormal_enable && touch_cnt) {
		if (data->atch.bigpalm_enable) {
			...
			__mxt_debug_msg(data,
					"ATCH: BIG TCHAREA=%d ATCHAREA=%d SUM=%d TCNT=%d\n",
					tch_area, atch_area, sum_size, touch_cnt);
			/* Coin */
			if ((touch_cnt == 1) && (tch_area < 5) && (sum_size > 7)
				&& ((sum_size - data->t9_size) > 4)
				&& (abn_case2++ > 50)) {
				__mxt_debug_msg(data, "ATCH: CASE 3-2 %d\n", abn_case2);
				mxt_check_command_calibration(data);
			}
			time_diff = TIME_WRAP_AROUND(data->atch.stylus_time, curr_time);
			if ((time_diff < 200) && (!data->t9_size)) {
				__mxt_debug_msg(data, "ATCH: Stylus After Contmin\n");
				mxt_check_command_calibration(data);
			}
		}
		/* Antitouch > finger count*X */
		if ((touch_cnt > 0) && (atch_area > touch_cnt * 11)
				&& (abn_case1++ > 20)) {
			__mxt_debug_msg(data, "ATCH: CASE 3-1 %d\n", abn_case1);
			mxt_check_command_calibration(data);
		}
		/* Stylus with Antitouch */
		if ((touch_cnt == 1) && (tch_area == 0) &&
				(atch_area > 1) && (abn_case3++ > 10)) {
			__mxt_debug_msg(data, "ATCH: CASE 3-3 %d\n", abn_case3);
			mxt_check_command_calibration(data);
		}
		/* 3finger: Antitouch > (Touch Area + X) */
		if ((touch_cnt < 6) && (atch_area > (tch_area + (touch_cnt+1)))
				&& (abn_case4++ > 20)) {
			__mxt_debug_msg(data, "ATCH: CASE 3-4 %d\n", abn_case4);
			mxt_check_command_calibration(data);
		}
		/*Big Sumsize > 150, finger < 6, tch_area < 50 */
		if ((sum_size > 150) && (touch_cnt < 6)
				&& (tch_area < 50) && (abn_case5++ > 20)) {
			__mxt_debug_msg(data, "ATCH: CASE 3-5 %d\n", abn_case5);
			mxt_check_command_calibration(data);
		}
		if ((sum_size > 200) && (touch_cnt > 3) && (tch_area > 40)
				&& (atch_area > 40) && (abn_case6++ > 20)) {
			__mxt_debug_msg(data, "ATCH: CASE 3-6 %d\n", abn_case6);
			mxt_check_command_calibration(data);
		}
		if ((sum_size > 90) && (touch_cnt > 3) && (tch_area < 30)
				&& (atch_area < 30) && (abn_case7++ > 20)) {
			__mxt_debug_msg(data, "ATCH: CASE 3-7 %d\n", abn_case7);
			mxt_check_command_calibration(data);
		}
		if ((touch_cnt > 2) && (touch_cnt < 6) && (atch_area > tch_area)
			&& (sum_size > (tch_area + 20)) && (abn_case8++ > 20)) {
			__mxt_debug_msg(data, "ATCH: CASE 3-8 %d\n", abn_case8);
			mxt_check_command_calibration(data);
		}
	}
}
...
#if defined(CONFIG_TOUCHSCREEN_ATMEL_AUTOCAL)
#define AUTOCAL_TIMER			100
#else
#define AUTOCAL_TIMER			0
#endif
						...
						struct mxt_message *message)
{
	...
	if (data->atch.bigpalm_enable) {
		...
		data->atch.bigpalm_enable = 0;
		mxt_set_T8_object_autocal(data, AUTOCAL_TIMER);
	}
	if (data->atch.calgood) {
		...
		data->atch.stylus_time = jiffies_to_msecs(jiffies);
	}
	if (data->atch.coin) {
