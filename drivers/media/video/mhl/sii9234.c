...
bool mhl_hpd_status;
EXPORT_SYMBOL(mhl_hpd_status);
struct completion mhl_hpd_complete;
EXPORT_SYMBOL(mhl_hpd_complete);
#endif
...
static int is_mhl_cable_connected(struct sii9234_data *sii9234)
{
		return sii9234->connector_status;
}
#endif
...
u8 mhl_onoff_ex(bool onoff)
{
	...
	if (onoff) {
		...
		if (sii9234->mhl_event_switch.state == 1) {
			pr_info("sii9234:MHL switch event sent : 0\n");
...
#ifdef CONFIG_SAMSUNG_USE_11PIN_CONNECTOR
		if (is_mhl_cable_connected(sii9234)) {
...
static int sii9234_cbus_init(struct sii9234_data *sii9234)
{
	...
	if (ret < 0)
		goto i2c_error_exit;
	/*only RGB fmt*/
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_VID_LINK_MODE, 0x01);
	if (ret < 0)
		goto i2c_error_exit;
...
static void goto_d3(void)
{
	...
	irq_set_irq_type(sii9234->pdata->mhl_tx_client->irq,
					 IRQ_TYPE_LEVEL_HIGH);
	sii9234_enable_irq();
...
void sii9234_check_connected_device(struct sii9234_data *sii9234, u8 value)
{
	/*read dev Category Regster*/
	sii9234->devcap.dev_type = MHL_FLD_GET(value, 3, 0);
	sii9234->devcap.pow = MHL_FLD_GET(value, 4, 4);
	/* {plim1, plim0}
	{0,0}  = device delivers at least 500mA
	{0,1}  = device delivers at least 900mA
	{1,0}  = device delivers at least 1.5A
	{1,1}  = Reserved */
	sii9234->devcap.plim = MHL_FLD_GET(value, 6, 5);
	pr_info("%s, DEV_CAT=PLIM:0x%x(%s), POW=%d, DEV_TYPE:0x%x\n", __func__,
		sii9234->devcap.plim, (sii9234->devcap.mhl_ver >= 0x20) ?  "ok"
		: "ignored", sii9234->devcap.pow, sii9234->devcap.dev_type);
}

static void sii9244_setup_charging(struct sii9234_data *sii9234)
{
	if (sii9234->devcap.mhl_ver >= 0x20) {
		sii9234->vbus_owner = sii9234->devcap.plim;
	} else {
		/*SAMSUNG DEVICE_ID 0x1134:dongle, 0x1234:dock */
		if ((sii9234->devcap.device_id == SS_MHL_DONGLE_DEV_ID ||
			sii9234->devcap.device_id == SS_MHL_DONGLE_DEV_ID) &&
					sii9234->devcap.reserved_data == 0x2)
			/*refer to plim table*/
			sii9234->vbus_owner = 1;
		else
			sii9234->vbus_owner = 3; /*300mA*/
	}
	...
	list_for_each_entry_safe(p_msc_pkt, scratch,
		...
		case CBUS_READ_DEVCAP:
			...
			case DEVCAP_MHL_VERSION:
				...
				break;
			case DEVCAP_DEV_CAT:
				sii9234_check_connected_device(sii9234, value);
				break;
...
/*end of this*/
	...
	if (ret == 0) {
		sii9234->vbus_owner = 3; /*UNKNOWN, 300mA*/
		pr_debug("dcap_timeout err, dcap_staus:%d\n",
			 sii9234->dcap_ready_status);
	} else
		sii9244_setup_charging(sii9234);
...
static void sii9234_extcon_work(struct work_struct *work)
{
	...
	struct sii9234_data *sii9234 = cable->sii9234;

	if (sii9234->extcon_mhl_attach) {
		wake_lock(&sii9234->mhl_wake_lock);
		pr_info("sii9234: power on, %s\n", __func__);
		sii9234->connector_status = true;
		ret = mhl_onoff_ex(true);
	} else {
		pr_info("sii9234: power down, %s\n", __func__);
		sii9234->connector_status = false;
		ret = mhl_onoff_ex(false);
				...
				unsigned long event, void *ptr)
{
	...
	if (!strcmp(extcon_cable_name[cable->cable_type], "MHL")) {
		...
		cable->sii9234->extcon_mhlvb_attach = event;
	}

	return NOTIFY_DONE;
}
#else
static int sii9234_connector_callback(struct notifier_block *this,
				  unsigned long event, void *ptr)
{
	int ret = 0;
	...
	if (event) {
		pr_info("sii9234: power on\n");
		sii9234->connector_status = true;
		ret = mhl_onoff_ex(true);
	} else {
		pr_info("sii9234: power down\n");
		sii9234->connector_status = false;
		ret = mhl_onoff_ex(false);
	}
	return ret;
					      ...
					      const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ARRAY_SIZE(support_cable_list); i++) {
		...
		INIT_WORK(&cable->work, sii9234_extcon_work);
		cable->nb.notifier_call = sii9234_extcon_notifier;
		cable->sii9234 = sii9234;
...
#else
	sii9234->mhl_nb.notifier_call = sii9234_connector_callback;
	if (sii9234->pdata->reg_notifier)
		sii9234->pdata->reg_notifier(&sii9234->mhl_nb);
#endif
	return 0;
#ifdef CONFIG_EXTCON
err_extcon:
...
#endif
	wake_lock_destroy(&sii9234->mhl_wake_lock);
	switch_dev_unregister(&sii9234->mhl_event_switch);
 err_exit2b:
...
err_exit2:
	...
	mutex_destroy(&sii9234_irq_lock);
