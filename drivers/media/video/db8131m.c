...
static const struct db8131m_regs reg_datas = {
	...
	.return_preview = {.name = "db8131m_stop_to_preview"},
	...
	.return_preview = {.reg = db8131m_stop_to_preview,
			.array_size = ARRAY_SIZE(db8131m_stop_to_preview)},
...
static int db8131m_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if ((state->runmode == DB8131M_RUNMODE_CAPTURE_STOP) ||
		(state->check_stream_off == 1)) {
		err |= db8131m_set_from_table(sd, "return_preview",
					&state->regs->return_preview, 1, 0);
	} else {
...
{
	return sprintf(buf, "%s", "DB8131A DB8131A\n");
}
